...
export const ProjectProposalEdit = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const loadAllData = async () => {
      try {
        const [orgs, cats, stats, currs, proposal] = await Promise.all([
          ...
          projectProposalService.getProposalById(id),
        ]);

        setOrganizations(orgs.organizations || orgs.rows || []);
        setCategories(cats.projectCategories || cats.rows || []);
        setStatuses(stats.proposalStatuses || stats.rows || []);
        setCurrencies(currs.currencies || currs.rows || []);
        ...
        setFormData({
          ...
          proposedCapitalAmount: proposal.proposed_capital_amount ?? '',
          currencyId: proposal.currency_id || '',
          remarks: proposal.remarks || '',
        });
      } catch (err) {
        enqueueSnackbar('Failed to load proposal details or lookup lists', { variant: 'error' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await projectProposalService.updateProposal(id, payload);
      enqueueSnackbar('Project proposal updated successfully', { variant: 'success' });
      navigate(`/projects/proposals/${id}`);
    } catch (err) {
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2.5, flexWrap: 'wrap', gap: 1.5 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
          ...
          <Typography variant="subtitle1" sx={{ fontWeight: 700, color: '#0f172a', fontSize: '0.95rem', lineHeight: 1 }}>
            Edit: {formData.proposedProjectName}
          </Typography>
        </Box>
        ...
        <Button
          ...
          onClick={() => navigate(`/projects/proposals/${id}`)}
...
export default ProjectProposalEdit;