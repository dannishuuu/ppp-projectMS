...
import { buildingTypesService } from '../../services/foundationService';
...
export const BuildingTypePage = () => {
  ...
  // Fetch Building Types List
  const fetchTypes = async () => {
    setLoading(true);
    try {
      const result = await buildingTypesService.getBuildingTypes({
        page: page + 1,
        limit: rowsPerPage,
        search: searchTerm,
        status: statusFilter,
      });
      setTypes(result.buildingTypes || result.rows || []);
      setTotalCount(result.pagination?.total || result.total || 0);
    } catch (error) {
      enqueueSnackbar(error.message || 'Failed to load building types', { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim()) {
      setFormError('Building type name is required.');
      return;
    }
    ...
    try {
      if (dialogMode === 'add') {
        await buildingTypesService.createBuildingType(formData);
        enqueueSnackbar('Building type created successfully', { variant: 'success' });
      } else if (dialogMode === 'edit' && selectedType) {
        await buildingTypesService.updateBuildingType(selectedType.id, formData);
        enqueueSnackbar('Building type updated successfully', { variant: 'success' });
      }
      handleDialogClose();
      fetchTypes();
    } catch (error) {
      setFormError(error.message || `Failed to ${dialogMode === 'add' ? 'create' : 'update'} building type.`);
    } finally {
  ...
  const handleToggleConfirm = async () => {
    ...
    try {
      const result = await buildingTypesService.toggleBuildingTypeStatus(selectedType.id);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const result = await buildingTypesService.deleteBuildingType(selectedType.id);
      enqueueSnackbar(result.message || 'Building type deleted successfully', { variant: 'success' });
      setDeleteDialogOpen(false);
      fetchTypes();
    } catch (error) {
      enqueueSnackbar(error.message || 'Failed to delete building type', { variant: 'error' });
    } finally {
  ...
  const getDialogTitle = () => {
    switch (dialogMode) {
      case 'add': return 'Add Building Type';
      case 'edit': return 'Edit Building Type';
      case 'view': return 'Building Type Details';
      default: return '';
    }
  };
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, width: '100%' }}>
      ...
      >
        ...
        <TextField
          size="small"
          placeholder="Search building type by name..."
          value={searchTerm}
        ...
        <TableContainer>
          <Table size="small" sx={{ minWidth: 650 }}>
            <TableHead sx={{ backgroundColor: '#f8fafc' }}>
              <TableRow>
                <TableCell sx={{ fontWeight: 700, color: '#475569', py: 1, fontSize: '0.78rem' }}>Building Type Name</TableCell>
                  ...
                  <TableRow key={type.id} hover sx={{ '& td': { py: 0.75 } }}>
                    ...
                    <TableCell align="right" sx={{ pr: 1 }}>
                      <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.5 }}>
                        ...
                        <Tooltip title="Edit Building Type" arrow placement="top">
                          <IconButton
                            size="small"
                        ...
                        {/* <Tooltip title="Delete Building Type" arrow placement="top">
                          <IconButton
                            size="small"
          ...
          {dialogMode === 'view' && selectedType ? (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              >
                <Avatar sx={{ width: 56, height: 56, backgroundColor: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(10px)' }}>
                  <BuildingIcon sx={{ fontSize: 32, color: 'white' }} />
                </Avatar>
                <Box sx={{ flex: 1 }}>
              ...
              <Box sx={{ px: 1 }}>
                ...
                >
                  <Typography variant="body2" sx={{ color: '#334155', lineHeight: 1.7, whiteSpace: 'pre-line' }}>
                    {selectedType.description || 'No description provided for this building type.'}
                  </Typography>
                </Box>
              </Box>
            ...
            <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, pt: 0.5 }}>
              ...
              <TextField
                ...
                rows={3}
                label="Description"
                placeholder="Describe this building type and its characteristics..."
      ...
      <ConfirmationModal
        ...
        title={selectedType?.is_active ? 'Deactivate Building Type' : 'Activate Building Type'}
        message={`Are you sure you want to ${selectedType?.is_active ? 'deactivate' : 'activate'} the building type "${selectedType?.name}"?`}
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to delete the building type "${selectedType?.name}"? This action cannot be undone.`}
        confirmText="Delete"
