...
export const AreaUnitPage = () => {
  ...
  // Fetch Area Units List
  const fetchAreaUnits = async () => {
    setLoading(true);
    try {
      const result = await areaUnitsService.getAreaUnits({
        page: page + 1,
        limit: rowsPerPage,
        search: searchTerm,
        status: statusFilter,
      });
      const dataList = result?.areaUnits || result?.rows || (Array.isArray(result) ? result : []);
      const total = result?.pagination?.total ?? result?.total ?? dataList.length;
      setAreaUnits(dataList);
      setTotalCount(total);
    } catch (error) {
      enqueueSnackbar(error.message || 'Failed to load area units', { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDialogOpen = (mode, unit = null) => {
    setDialogMode(mode);
    setSelectedUnit(unit);
    if ((mode === 'edit' || mode === 'view') && unit) {
      setFormData({
        name: unit.name || '',
        code: unit.code || '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim()) {
      setFormError('Area unit name is required.');
      return;
    }
    if (!formData.code.trim()) {
      setFormError('Area unit code is required.');
      return;
    }
    ...
    try {
      if (dialogMode === 'add') {
        await areaUnitsService.createAreaUnit(payload);
        enqueueSnackbar('Area unit created successfully', { variant: 'success' });
      } else if (dialogMode === 'edit' && selectedUnit) {
        await areaUnitsService.updateAreaUnit(selectedUnit.id, payload);
        enqueueSnackbar('Area unit updated successfully', { variant: 'success' });
      }
      handleDialogClose();
      fetchAreaUnits();
    } catch (error) {
      setFormError(error.message || `Failed to ${dialogMode === 'add' ? 'create' : 'update'} area unit.`);
    } finally {
  ...
  // Toggle Status
  const handleToggleDialogOpen = (unit) => {
    setSelectedUnit(unit);
  ...
  const handleToggleConfirm = async () => {
    ...
    try {
      const result = await areaUnitsService.toggleAreaUnitStatus(selectedUnit.id);
  ...
  const getDialogTitle = () => {
    switch (dialogMode) {
      case 'add': return 'Add Area Unit';
      case 'edit': return 'Edit Area Unit';
      case 'view': return 'Area Unit Details';
      default: return '';
    }
  };
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, width: '100%' }}>
          ...
          <Table sx={{ minWidth: 650 }}>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={8} align="center" sx={{ py: 6 }}>
                    <AreaIcon sx={{ fontSize: 44, color: '#cbd5e1', mb: 1 }} />
                    ...
                    <Typography variant="caption" sx={{ color: '#94a3b8' }}>
                      {searchTerm || statusFilter !== 'all'
                        ...
                        : 'Click "Add Area Unit" above to create one.'}
                    </Typography>
                  </TableCell>
                </TableRow>
              ) : (
                    ...
                    <TableCell sx={{ py: 1.5, textAlign: 'right' }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 0.5 }}>
                        <Tooltip title="View Details" arrow placement="top">
                          <IconButton
                            size="small"
                            onClick={() => handleDialogOpen('view', unit)}
                        ...
                        <Tooltip title="Edit Area Unit" arrow placement="top">
                          <IconButton
                            size="small"
                            onClick={() => handleDialogOpen('edit', unit)}
                        ...
                        <Tooltip title={unit.is_active ? 'Deactivate' : 'Activate'} arrow placement="top">
                          <IconButton
                            size="small"
                            onClick={() => handleToggleDialogOpen(unit)}
                            sx={{
                              p: 0.5,
                              color: unit.is_active ? '#eab308' : '#16a34a',
                              '&:hover': {
                                backgroundColor: unit.is_active ? '#fef9c3' : '#dcfce7',
                              },
                            }}
                          >
                            {unit.is_active ? (
          ...
          {dialogMode === 'view' && selectedUnit ? (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              >
                <Avatar sx={{ width: 56, height: 56, backgroundColor: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(10px)' }}>
                  <AreaIcon sx={{ fontSize: 32, color: 'white' }} />
                </Avatar>
                <Box sx={{ flex: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap', mb: 1 }}>
                    {selectedUnit.name_amharic && (
                      <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.9)', fontWeight: 500 }}>
                        🇪🇹 {selectedUnit.name_amharic}
                      </Typography>
                    )}
                    {selectedUnit.name_afaan_oromo && (
                      <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.9)', fontWeight: 500 }}>
                        🌐 {selectedUnit.name_afaan_oromo}
                      </Typography>
                    )}
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                    <Chip
                      label={selectedUnit.is_active ? 'Active' : 'Inactive'}
                      size="small"
                      sx={{
                        height: 22,
                        fontSize: '0.7rem',
                        fontWeight: 700,
                        backgroundColor: selectedUnit.is_active ? 'rgba(16, 185, 129, 0.9)' : 'rgba(239, 68, 68, 0.9)',
                        color: 'white',
              ...
              <Box sx={{ px: 1 }}>
                ...
                >
                  <Typography variant="body2" sx={{ color: '#334155', lineHeight: 1.7, whiteSpace: 'pre-line' }}>
                    {selectedUnit.description || 'No description provided for this area unit.'}
                  </Typography>
                </Box>
              </Box>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    <Box sx={{ p: 2, borderRadius: 2, backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' }}>
                      ...
                      <Typography variant="body2" sx={{ fontWeight: 600, color: '#334155' }}>
                        {selectedUnit.created_by_name || 'System'}
                      </Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={6}>
                  ...
                  <Grid item xs={6}>
                    <Box sx={{ p: 2, borderRadius: 2, backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' }}>
                      ...
                      <Typography variant="body2" sx={{ fontWeight: 600, color: '#334155' }}>
                        {selectedUnit.updated_by_name || 'System'}
                      </Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={6}>
            ...
            <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, pt: 0.5 }}>
              <TextField
                ...
                placeholder="e.g. Square Meter, Hectare, Square Feet"
                value={formData.name}
              ...
              <TextField
                ...
                fullWidth
                label="Unit Code"
                placeholder="e.g. SQM, HA, SQFT, M2"
                value={formData.code}
                ...
                helperText={
                  dialogMode === 'edit'
                    ? 'Unit code is permanent and cannot be modified'
              ...
              <TextField
                ...
                placeholder="e.g. Kaareemeetira, Hektaara"
              ...
              <TextField
                ...
                rows={3}
                label="Description"
                placeholder="Describe this area measurement unit..."
      ...
      <ConfirmationModal
        ...
        title={selectedUnit?.is_active ? 'Deactivate Area Unit' : 'Activate Area Unit'}
        message={
          selectedUnit?.is_active
            ? `Are you sure you want to deactivate "${selectedUnit?.name}" (${selectedUnit?.code})? It may not be available for selection in new buildings or units.`
            : `Are you sure you want to activate "${selectedUnit?.name}" (${selectedUnit?.code})?`
        }
        confirmText={selectedUnit?.is_active ? 'Deactivate' : 'Activate'}
        cancelText="Cancel"
        confirmColor={selectedUnit?.is_active ? 'warning' : 'primary'}
