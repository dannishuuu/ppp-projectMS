...
export const BuildingCreatePage = () => {
  ...
  useEffect(() => {
    const fetchLookups = async () => {
      try {
        const [btRes, regRes, auRes] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      enqueueSnackbar('Building registered successfully', { variant: 'success' });
      navigate(result?.id ? `/buildings/${result.id}` : '/buildings');
    } catch (err) {
      setErrorMsg(err.message || 'Failed to create building.');
    } finally {
      setSaving(false);
    }
  };
